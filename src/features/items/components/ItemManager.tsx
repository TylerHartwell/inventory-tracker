...
import { ItemInput } from "@/features/items/components/ItemInput"
import { useItemsRealtime } from "@/features/items/hooks/useItemsRealtime"
import { useEffect, useMemo, useState } from "react"
import { SortField } from "@/shared/components/SortOrderSelect"
...
import BulkDeleteModal from "@/shared/components/BulkDeleteModal"
import FilterSection, { ImageFilterMode, OptionalFilterType } from "@/features/lists/components/FilterSection"
import DisplaySection, { VisibilityMode } from "@/shared/components/DisplaySection"
import ActionSection from "@/shared/components/ActionSection"
import { getDateOnlySortValue } from "@/shared/utils/dateOnly"
...
function ItemManager({ session, onLogout }: { session: Session; onLogout: () => Promise<void> }) {
  ...
  return (
    ...
    <div className="max-w-[clamp((100%*5/6),calc(100%-((100vw-768px)/(1024px-768px))*(100%-(100%*5/6))),100%)] mx-auto p-2 flex flex-col gap-2 ">
      <Header userId={session.user.id} userEmail={session.user.email ?? null} onLogout={onLogout} />
      <ItemInput
        session={session}
